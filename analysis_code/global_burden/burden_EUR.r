...
data_name <- c('EUR',as.character(dat))
...
system('mkdir -p burden_loop_EUR/')
...
## =============== Filter by data, CNV type, and CNV regions first
  ...
  system(paste("plink --noweb --cfile ",input," ",data2," ",type2," ",region2," --cnv-write --out burden_loop_EUR/type_loop",sep=""))
  ## gene cnv-make-map
  system("plink --noweb --cfile burden_loop_EUR/type_loop --cnv-make-map --out burden_loop_EUR/type_loop")


  ## --- Frequency pruning    
  system(paste("plink --noweb --cfile burden_loop_EUR/type_loop ",freq2," --cnv-write --out burden_loop_EUR/freq_loop",sep=""))
  ## gene cnv-make-map
  system("plink --noweb --cfile burden_loop_EUR/freq_loop --cnv-make-map --out burden_loop_EUR/freq_loop")

  ## --- Size pruning
  system(paste("plink --noweb --cfile burden_loop_EUR/freq_loop ",size2," --cnv-write --out burden_loop_EUR/size_loop",sep=""))
  system("plink --noweb --cfile burden_loop_EUR/size_loop --cnv-make-map --out burden_loop_EUR/size_loop")
 
  ## Adding gene/exon count separately

  system(paste("plink --noweb --cfile burden_loop_EUR/size_loop --cnv-indiv-perm --mperm ",perm," --cnv-count ",gene_file," --out burden_loop_EUR/burden_loop",sep=""))
...
## Check if any segments were matched
                         ...
                         cnv <- read.table('burden_loop_EUR/size_loop.cnv',h=T) 
...
## == PLINK results
plink_sum <- read.table("burden_loop_EUR/burden_loop_platform.cnv.grp.summary",h=T)
plink_emp <- read.table("burden_loop_EUR/burden_loop_platform.cnv.summary.mperm",h=T)

## PART 3.5: CNV burden analysis in R

indiv <- read.table("burden_loop_EUR/burden_loop_platform.cnv.indiv",h=T)
...
comrg_cnv2 <-  comrg[comrg$NSEG > 0,]
CNV_CARRIER_SAMPLE_SIZE[X] <- nrow(comrg_cnv2)
CNV_cas_CARRIER_SAMPLE_SIZE[X] <- nrow(comrg_cnv2[comrg_cnv2$PHE==2,])
CNV_con_CARRIER_SAMPLE_SIZE[X] <- nrow(comrg_cnv2[comrg_cnv2$PHE==1,])
CNV_GENIC_CARRIER_SAMPLE_SIZE[X] <- nrow(comrg[comrg$GENIC_CNV_COUNT > 0,])
...
## ==== KB burden
comrg$GENIC_KB <-  (comrg$GENIC_KB)/100
...
write.table(full_data,paste(sep='',output,'.burden'),col=T,row=F,quo=F,sep='\t')
