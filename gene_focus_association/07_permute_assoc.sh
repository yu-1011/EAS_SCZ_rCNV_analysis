...
phe_file=${WORKING_DIR}/input/cnv_bed/random_phe_${SGE_TASK_ID}.phe
cnv_type=$1
...
regenie \
  --step 2 \
  --bt \
  --bed ${WORKING_DIR}/input/cnv_bed/cnv_permute.${cnv_type} \
  --covarFile ${phe_file} \
  --phenoFile ${phe_file} \
  --bsize 2 \
  --firth \
  ...
  --out ${WORKING_DIR}/output/permute/${SGE_TASK_ID}_${cnv_type}.step2 \
  --catCovarList CNV_platform \
